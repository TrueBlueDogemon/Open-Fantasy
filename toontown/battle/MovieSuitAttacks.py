...
def getToonTrack(attack, damageDelay=1e-06, damageAnimNames=None, dodgeDelay=0.0001, dodgeAnimNames=None,
                 splicedDamageAnims=None, splicedDodgeAnims=None, target=None, showDamageExtraTime=0.01, showMissedExtraTime=0.5):
    if not target:
        target = attack['target']
    toon = target['toon']
    battle = attack['battle']
...
def getPropTrack(prop, parent, posPoints, appearDelay, remainDelay, scaleUpPoint=Point3(1), scaleUpTime=0.5,
                 scaleDownTime=0.5, startScale=Point3(0.01), anim=0, propName='none', animDuration=0.0, animStartTime=0.0):
    if anim == 1:
        track = Sequence(
            Wait(appearDelay),
            Func(
...
def getPropAppearTrack(prop, parent, posPoints, appearDelay, scaleUpPoint=Point3(
        1), scaleUpTime=0.5, startScale=Point3(0.01), poseExtraArgs=None):
    propTrack = Sequence(
        Wait(appearDelay),
        Func(
            __showProp,
...
def getThrowTrack(object, target, duration=1.0,
...
def getSplicedLerpAnims(animName, origDuration,
                        newDuration, startTime=0, fps=30, reverse=0):
    anims = []
    addition = 0
    ...
    tiePropTrack.append(
        getPropThrowTrack(
            attack, tie, [
                ...
                __toonGroundPoint(
                    attack, toon, 0.1)], hitDuration=0.4, missDuration=0.8, missScaleDown=1.2))
...
def doPoundKey(attack):
    ...
    partTrack = getPartTrack(
        particleEffect, 2.1, 1.55, [
            particleEffect, suit, 0])
    suitType = getSuitBodyType(attack['suitName'])
    if suitType == 'a':
        phonePosPoints = [Point3(-0.23, 0.01, -0.26),
                          VBase3(5.939, 2.763, -177.591)]
        receiverPosPoints = [
            Point3(-0.13, -0.07, -0.06), VBase3(-1.854, 2.434, -177.579)]
        ...
        phonePosPoints = [Point3(0.23, 0.17, -0.11),
                          VBase3(5.939, 2.763, -177.591)]
        receiverPosPoints = [
    ...
    if suitType == 'a' or suitType == 'b':
        propTrack.append(
            LerpScaleInterval(
                ...
                0.01,
                receiverAdjustScale))
        propTrack.append(LerpPosHprInterval(
...
def doShred(attack):
    ...
    suitTrack = getSuitTrack(attack)
    partTrack = getPartTrack(
        particleEffect, 3.5, 1.9, [
            particleEffect, suit, 0])
    paperPosPoints = [Point3(0.59, -0.31, 0.81),
                      VBase3(79.224, 32.576, -179.449)]
    ...
    return Parallel(suitTrack, paperPropTrack, shredderPropTrack,
                    partTrack, toonTrack, soundTrack)
...
def doFillWithLead(attack):
    ...
    pencilPosPoints = [Point3(-0.29, -0.33, -0.13),
    ...
    headTrack = getPartTrack(
        headSmotherEffect, partDelay, partDuration, [
    ...
    if dmg > 0:
        ...
        return Parallel(suitTrack, pencilPropTrack, sharpenerPropTrack,
                        sprayTrack, headTrack, torsoTrack, legsTrack, colorTrack, toonTrack)
    else:
        return Parallel(suitTrack, pencilPropTrack,
                        sharpenerPropTrack, sprayTrack, toonTrack)
...
def doFountainPen(attack):
    ...
    propTrack += [LerpScaleInterval(pen,
                                    0.5,
                                    MovieUtil.PNT3_NEARZERO),
    ...
    toonTrack = getToonTrack(attack, 1.81, ['conked'], dodgeDelay=0.11, splicedDodgeAnims=[
                             ['duck', 0.01, 0.6]], showMissedExtraTime=1.66)
...
def doRubOut(attack):
    ...
    padPropTrack = getPropTrack(
        pad, suit.getLeftHand(), padPosPoints, 0.5, 2.57)
    pencilPosPoints = [Point3(0.04, -0.38, -0.1),
                       VBase3(-170.223, -3.762, -62.929)]
    pencilPropTrack = getPropTrack(
        pencil,
        suit.getRightHand(),
        pencilPosPoints,
    ...
    partDelay = 2.5
    headTrack = getPartTrack(headEffect, partDelay + 0,
                             0.5, [headEffect, toon, 0])
    torsoTrack = getPartTrack(
        torsoEffect, partDelay + 1.1, 0.5, [torsoEffect, toon, 0])
    legsTrack = getPartTrack(legsEffect, partDelay +
                             2.2, 0.5, [legsEffect, toon, 0])
...
def doWriteOff(attack):
    ...
    suitTrack = getSuitTrack(attack)
    padPosPoints = [Point3(-0.25, 1.38, -0.08),
                    VBase3(-19.078, -6.603, -171.594)]
    padPropTrack = getPropTrack(
        pad,
        suit.getLeftHand(),
        padPosPoints,
    ...
    def missPoint(
        ...
        checkmark,
        toon)
    pencilPosPoints = [Point3(-0.47, 1.08, 0.28),
                       VBase3(21.045, 12.702, -176.374)]
    ...
    pencilPropTrack.append(
        getPropThrowTrack(
            attack, checkmark, [
                __toonFacePoint(toon)], [missPoint]))
    ...
    soundTrack = Sequence(
        ...
        SoundInterval(
            ...
            node=suit))
    return Parallel(suitTrack, toonTrack, padPropTrack,
                    pencilPropTrack, soundTrack)
...
def doRubberStamp(attack):
    ...
    suitType = getSuitBodyType(attack['suitName'])
    if suitType == 'a':
        padPosPoints = [Point3(-0.65, 0.83, -0.04),
                        VBase3(5.625, 4.456, -165.125)]
        stampPosPoints = [Point3(-0.64, -0.17, -0.03), MovieUtil.PNT3_ZERO]
    elif suitType == 'c':
        padPosPoints = [Point3(0.19, -0.55, -0.21),
                        VBase3(-166.76, -4.001, -1.658)]
    ...
    propTrack = Sequence(
        Func(
            ...
            stamp, 0.5, MovieUtil.PNT3_ONE), Wait(2.6), Func(
                battle.movie.needRestoreRenderProp, cancelled), Func(
                    cancelled.reparentTo, render), Func(
                        cancelled.setScale, 0.6), Func(
                            cancelled.setPosHpr, stamp, 0.81, -1.11, -0.16, 0, 0, 90), Func(
...
def doRazzleDazzle(attack):
    ...
    return Sequence(Parallel(suitTrack, signPropTrack, signPropAnimTrack,
                    toonTrack, soundTrack), Func(MovieUtil.removeProp, sign))
...
def doSynergy(attack):
    ...
    if hitAtleastOneToon > 0:
        fallingSoundTrack = getSoundTrack(
            'Toon_bodyfall_synergy.ogg',
            delay=damageDelay + 0.5,
            node=suit)
...
def doTeeOff(attack):
    ...
    ballPropTrack = Sequence(
        getPropAppearTrack(
            ...
            battle.movie.needRestoreRenderProp, ball), Func(
                    ball.wrtReparentTo, render), Wait(2.15))
    ...
    return Parallel(suitTrack, toonTrack, clubPropTrack,
                    ballPropTrack, soundTrack)
...
def doBrainStorm(attack):
    ...
    cloudPropTrack.append(
        Parallel(
            ParticleInterval(
                snowEffect, cloud, worldRelative=0, duration=2.2, cleanup=True), Sequence(
    ...
    if dmg > 0:
        ...
        hprTrack = Sequence(
            Wait(
                ...
                can, throwDuration, canHpr), Wait(0.4), LerpHprInterval(
                can, 0.4, Point3(
                    83.27, 19.52, -177.92)), LerpHprInterval(
                        can, 0.3, Point3(
                            95.24, -72.09, 88.65)), LerpHprInterval(
                                can, 0.2, Point3(
                                    -96.34, -2.63, 179.89)))
        soundTrack = getSoundTrack(
            'SA_canned_tossup_only.ogg', delay=2.6, node=suit)
    canTrack = Sequence(
        Parallel(
...
def doPinkSlip(attack):
    ...
    throwDuration = 0.5
    suitTrack = getSuitTrack(attack)
    posPoints = [Point3(0.07, -0.06, -0.18),
                 VBase3(-172.075, -26.715, -89.131)]
...
def doReOrg(attack):
    ...
    if dmg > 0:
        ...
        def getChestTrack(part, attackDelay=attackDelay):
            ...
            return Sequence(Wait(attackDelay), LerpHprInterval(part, 1.1, VBase3(
                180, 0, 0)), Wait(1.1), LerpHprInterval(part, 1.1, part.getHpr()))
...
def doSacked(attack):
    ...
    if dmg > 0:
        ...
        sackAppearTrack.append(
            LerpScaleInterval(
                sack, 0.3, MovieUtil.PNT3_NEARZERO))
        sackTrack = Sequence(
...
def doGlowerPower(attack):
    ...
    return Parallel(suitTrack, toonTrack, soundTrack,
                    leftKnifeTracks, rightKnifeTracks)
...
def doHeadShrink(attack):
    ...
    if dmg > 0:
        soundTrack = Sequence(
            Wait(2.1),
            SoundInterval(
                globalBattleSoundCache.getSound('SA_head_shrink_only.ogg'),
                duration=2.1,
                node=suit),
            Wait(1.6),
            SoundInterval(
                globalBattleSoundCache.getSound('SA_head_grow_back_only.ogg'),
                node=suit))
...
def doRolodex(attack):
    ...
    suitType = getSuitBodyType(attack['suitName'])
    if suitType == 'a':
        propPosPoints = [Point3(-0.51, -0.03, -0.1),
                         VBase3(89.673, 2.166, 177.786)]
        ...
        propPosPoints = [
            Point3(
                0.12, 0.24, 0.01), VBase3(
                99.032, 5.973, -179.839)]
    ...
    partTrack3 = getPartTrack(
        particleEffect3, part3Delay, part3Duration, [
    ...
    return Parallel(suitTrack, toonTrack, propTrack,
                    soundTrack, partTrack2, partTrack3)
...
def doPlayHardball(attack):
    ...
    if suitType == 'a':
        ...
        dodgeDelay = 1.86
    suitTrack = getSuitTrack(attack)
    ballPosPoints = [Point3(0.04, 0.03, -0.31),
    ...
    if dmg > 0:
        ...
        soundTrack = getSoundTrack(
            'SA_hardball_impact_only.ogg', delay=2.8, node=suit)
    else:
...
def doPowerTie(attack):
    ...
    tiePropTrack.append(
        getPropThrowTrack(
            attack, tie, [
                __toonFacePoint(toon)], [
                __toonGroundPoint(
                    attack, toon, 0.1)], hitDuration=0.4, missDuration=0.8))
    toonTrack = getToonTrack(
        attack,
        damageDelay,
        ['conked'],
    ...
    if dmg > 0:
        hitSound = getSoundTrack(
            'SA_powertie_impact.ogg', delay=2.9, node=suit)
...
def doCigarSmoke(attack):
    ...
    propTrack = getPropTrack(cigar, suit.getRightHand(), [
                             Point3(-0.1, 0.0, -0.2), VBase3(180.0, 0.0, 0.0)], 0.1, 5.0, Point3(5.0, 5.0, 5.0))
    BattleParticles.loadParticles()
...
def doFloodTheMarket(attack):
    ...
    soundTrack = Sequence(
        Wait(0.9),
        SoundInterval(
            globalBattleSoundCache.getSound('SA_synergy.ogg'),
            node=suit))
    return Parallel(suitTrack, partTrack, waterfallTrack,
...
def doDoubleTalk(attack):
    ...
    suitTrack = getSuitTrack(attack)
    partTrack = getPartTrack(
        particleEffect, partDelay, 1.8, [
            particleEffect, suit, 0])
    partTrack2 = getPartTrack(
        particleEffect2, partDelay, 1.8, [
            particleEffect2, suit, 0])
    damageAnims = [['duck',
                    0.01,
                    0.4,
...
def doHotAir(attack):
    ...
    sprayTrack = getPartTrack(
        sprayEffect, sprayDelay, 2.3, [
            sprayEffect, suit, 0])
    baseFlameTrack = getPartTrack(
        baseFlameEffect, flameDelay, flameDuration, [
            baseFlameEffect, toon, 0])
    flameTrack = getPartTrack(
        flameEffect, flameDelay, flameDuration, [
            flameEffect, toon, 0])
    flecksTrack = getPartTrack(
        flecksEffect, flecksDelay, flecksDuration, [
            flecksEffect, toon, 0])
...
def doPickPocket(attack):
    ...
    suitTrack = getSuitTrack(attack)
    billPosPoints = [Point3(-0.01, 0.45, -0.25),
                     VBase3(136.424, -46.434, -129.712)]
    ...
    if suitType == 'a' or suitType == 'b':
        propTrack.append(
            LerpScaleInterval(
                ...
                receiverAdjustScale))
        propTrack.append(LerpPosHprInterval(
            receiver, 0.00005, Point3(-0.53, 0.21, -0.54), VBase3(-99.49, -35.27, 1.84)))
    else:
        propTrack.append(LerpPosHprInterval(
            receiver, 0.00005, Point3(-0.45, 0.48, -0.62), VBase3(-87.47, -18.21, 7.82)))
    propTrack.append(Wait(dialDuration))
    propTrack.append(Func(receiver.wrtReparentTo, phone))
    propTrack.append(Wait(finalPhoneDelay))
...
def doFilibuster(attack):
    ...
    suitTrack = getSuitTrack(attack)
    sprayTrack = getPartTrack(
        sprayEffect, partDelay, partDuration, [
            sprayEffect, suit, 0])
    sprayTrack2 = getPartTrack(sprayEffect2, partDelay +
                               0.8, partDuration, [sprayEffect2, suit, 0])
    sprayTrack3 = getPartTrack(sprayEffect3, partDelay +
                               1.6, partDuration, [sprayEffect3, suit, 0])
    sprayTrack4 = getPartTrack(sprayEffect4, partDelay +
                               2.4, partDuration, [sprayEffect4, suit, 0])
    damageAnims = []
    for i in range(0, 4):
        damageAnims.append(['cringe',
                            1e-05,
...
def doSchmooze(attack):
    ...
    for i in range(0, 4):
        upperPartTracks.append(getPartTrack(
            upperEffects[i], partDelay + i * 0.65, 0.8, [upperEffects[i], suit, 0]))
        lowerPartTracks.append(getPartTrack(
            lowerEffects[i], partDelay + i * 0.65 + 0.7, 1.0, [lowerEffects[i], suit, 0]))
...
def doRedTape(attack):
    ...
    suitType = getSuitBodyType(attack['suitName'])
    if suitType == 'a':
        tapePosPoints = [Point3(-0.24, 0.09, -0.38),
                         VBase3(-1.152, 86.581, -76.784)]
    else:
        tapePosPoints = [Point3(0.24, 0.09, -0.38),
                         VBase3(-1.152, 86.581, -76.784)]
...
def doParadigmShift(attack):
    ...
    for t in targets:
        ...
        if dmg > 0:
            ...
            shadowTrack.append(
                LerpScaleInterval(
                    fakeShadow, 0.4, Point3(
                        0.17, 0.17, 0.17)))
            shadowTrack.append(Wait(1.81))
            shadowTrack.append(
                LerpScaleInterval(
                    fakeShadow, 0.1, Point3(
                        0.27, 0.27, 0.27)))
    ...
    if hitAtleastOneToon == 1:
        soundTrack = getSoundTrack(
            'SA_paradigm_shift.ogg', delay=2.1, node=suit)
        return Parallel(suitTrack, sprayTrack, soundTrack,
                        liftTracks, toonTracks, toonRiseTracks)
    else:
        return Parallel(suitTrack, sprayTrack, liftTracks,
...
def doPowerTrip(attack):
    ...
    def getPowerTrack(effect, suit=suit, battle=battle):
        partTrack = Sequence(
            Wait(1.0), Func(
                ...
                effect.start, suit), Wait(0.4), LerpPosInterval(
                effect, 1.0, Point3(
                    0, 15, 0.4)), LerpFunctionInterval(
                        effect.setAlphaScale, fromData=1, toData=0, duration=0.4), Func(
                            effect.cleanup), Func(
    ...
    waterfallTrack = getPartTrack(
        waterfallEffect, 0.6, 1.3, [
            waterfallEffect, suit, 0])
    toonTracks = getToonTracks(attack, 1.8, ['slip-forward'], 1.29, ['jump'])
    return Parallel(suitTrack, partTrack1, partTrack2,
...
def doBounceCheck(attack):
    ...
    if hitSuit:
        ...
        checkPropTrack.append(
            LerpScaleInterval(
                check, 0.3, MovieUtil.PNT3_NEARZERO))
...
def doFired(attack):
    ...
    if dmg > 0:
        return Parallel(suitTrack, baseFlameTrack, flameTrack,
                        flecksTrack, toonTrack, colorTrack, soundTrack)
    else:
        return Parallel(suitTrack, baseFlameSmallTrack,
                        flameSmallTrack, flecksSmallTrack, toonTrack, soundTrack)
...
def doAudit(attack):
    ...
    suitTrack = getSuitTrack(attack)
    partTrack = getPartTrack(
        particleEffect, 2.1, 1.9, [
            particleEffect, suit, 0])
    partTrack2 = getPartTrack(
        particleEffect2, 2.2, 2.0, [
            particleEffect2, suit, 0])
    partTrack3 = getPartTrack(
        particleEffect3, 2.3, 2.1, [
            particleEffect3, suit, 0])
    partTrack4 = getPartTrack(
        particleEffect4, 2.4, 2.2, [
            particleEffect4, suit, 0])
    partTrack5 = getPartTrack(
        particleEffect5, 2.5, 2.3, [
            particleEffect5, suit, 0])
    suitType = getSuitBodyType(attack['suitName'])
    if suitType == 'a':
        calcPosPoints = [Point3(-0.15, 0.37, 0.03),
                         VBase3(1.352, -6.518, -6.045)]
        calcDuration = 0.76
        scaleUpPoint = Point3(1.1, 1.85, 1.81)
    else:
        calcPosPoints = [
            Point3(
                0.35, 0.52, 0.03), VBase3(
                1.352, -6.518, -6.045)]
        calcDuration = 1.87
...
def doCrunch(attack):
    ...
    numberSpillTrack1 = getPartTrack(
        numberSpill1, 1.1, 2.2, [
            numberSpill1, suit, 0])
    numberSpillTrack2 = getPartTrack(
        numberSpill2, 1.5, 1.0, [
            numberSpill2, suit, 0])
    ...
    for i in range(0, numOfNumbers - 1):
        ...
        nextSprayTrack = getPartTrack(
            nextSpray, nextStartTime, nextDuration, [
                nextSpray, suit, 0])
    ...
    for i in range(0, numOfNumbers):
        ...
        numberTrack = Sequence(
            Wait(0.9), LerpScaleInterval(
                next, 0.6, MovieUtil.PNT3_ONE), Wait(1.7), Func(
...
def doLiquidate(attack):
    ...
    cloudPropTrack.append(
        Parallel(
            Sequence(
                ParticleInterval(
                    rainEffect, cloud, worldRelative=0, duration=2.1, cleanup=True)), Sequence(
                Wait(0.1), ParticleInterval(
                ...
                ActorInterval(
                    cloud, 'stormcloud', startTime=3, duration=0.1), ActorInterval(
                    cloud, 'stormcloud', startTime=1, duration=2.3))))
    cloudPropTrack.append(Wait(0.4))
    cloudPropTrack.append(
        LerpScaleInterval(
            cloud,
...
def doBite(attack):
    ...
    if dmg > 0:
        ...
        teethAppearTrack.append(
            LerpPosInterval(
                teeth, 0.4, pos=Point3(
                    x, y + 3.2, toonHeight * 0.7)))
        teethAppearTrack.append(
            LerpPosInterval(
                teeth, 0.3, pos=Point3(
                    x, y + 4.7, toonHeight * 0.5)))
        teethAppearTrack.append(Wait(0.2))
        teethAppearTrack.append(
            LerpPosInterval(
                teeth, 0.1, pos=Point3(
                    x, y - 0.2, toonHeight * 0.9)))
        teethAppearTrack.append(Wait(0.4))
        scaleTrack = Sequence(
            Wait(throwDelay), LerpScaleInterval(
                teeth, throwDuration, Point3(
                    8, 8, 8)), Wait(0.9), LerpScaleInterval(
                teeth, 0.2, Point3(
                    14, 14, 14)), Wait(1.2), LerpScaleInterval(
                        teeth, 0.3, MovieUtil.PNT3_NEARZERO))
        hprTrack = Sequence(
            Wait(throwDelay), LerpHprInterval(
                teeth, 0.3, Point3(
                    180, 0, 0)), Wait(0.2), LerpHprInterval(
                teeth, 0.4, Point3(
                    180, -35, 0), startHpr=Point3(
                        180, 0, 0)), Wait(0.6), LerpHprInterval(
                            teeth, 0.1, Point3(
        ...
        animTrack = Sequence(
            Wait(throwDelay), ActorInterval(
                ...
                teeth, 'teeth', duration=0.3), Func(
                teeth.pose, 'teeth', 1), Wait(0.7), ActorInterval(
                    teeth, 'teeth', duration=0.9))
        propTrack = Sequence(
            Parallel(
...
def doChomp(attack):
    ...
    if dmg > 0:
        ...
        teethAppearTrack.append(
            LerpPosInterval(
                teeth, 0.1, pos=Point3(
                    x, y - 1.2, toonHeight * 0.7)))
        teethAppearTrack.append(
            LerpPosInterval(
                teeth, 0.1, pos=Point3(
                    x, y - 0.7, toonHeight * 0.4)))
        teethAppearTrack.append(Wait(0.4))
        scaleTrack = Sequence(
            Wait(throwDelay), LerpScaleInterval(
                teeth, throwDuration, Point3(
                    6, 6, 6)), Wait(0.9), LerpScaleInterval(
                teeth, 0.2, Point3(
        ...
        teethAppearTrack.append(
            LerpScaleInterval(
                teeth, 0.6, MovieUtil.PNT3_NEARZERO))
        hprTrack = Sequence(
            Wait(throwDelay),
            LerpHprInterval(
                teeth,
...
def doUndergroundLiquidity(attack):
    ...
    if dmg > 0:
        ...
        puddleTrack = Sequence(
            Func(
                battle.movie.needRestoreRenderProp, puddle), Wait(
                damageDelay - 0.7), Func(
                puddle.reparentTo, battle), Func(
                    puddle.setPos, toon.getPos(battle)), LerpScaleInterval(
                        puddle, 1.7, Point3(
                            1.7, 1.7, 1.7), startScale=MovieUtil.PNT3_NEARZERO), Wait(3.2), LerpFunctionInterval(
                                puddle.setAlphaScale, fromData=1, toData=0, duration=0.8), Func(
                                    MovieUtil.removeProp, puddle), Func(
                                        battle.movie.clearRenderProp, puddle))
...
def doWithdrawal(attack):
    ...
    toonTrack = getToonTrack(attack, 1.2, ['cringe'], 0.2, splicedDodgeAnims=[
                             ['duck', 1e-05, 0.8]], showMissedExtraTime=0.8)
    headParts = toon.getHeadParts()
    torsoParts = toon.getTorsoParts()
    legsParts = toon.getLegsParts()
...
def doMumboJumbo(attack):
    ...
    partTrack2 = getPartTrack(
        particleEffect2, 2.5, 2, [
            particleEffect2, suit, 0])
    partTrack3 = getPartTrack(
        particleEffect3, 3.3, 1.7, [
            particleEffect3, toon, 0])
    partTrack4 = getPartTrack(
        particleEffect4, 3.3, 1.7, [
            particleEffect4, toon, 0])
    partTrack5 = getPartTrack(
        particleEffect5, 3.3, 1.7, [
...
def doSpin(attack):
    ...
    if dmg > 0:
        toonSpinTrack = Sequence(Wait(damageDelay + 0.9),
...
def doLegalese(attack):
    ...
    sprayTrack1 = getPartTrack(
        sprayEffect1, partDelay, partDuration, [
            sprayEffect1, suit, 0])
    sprayTrack2 = getPartTrack(sprayEffect2, partDelay +
                               0.8, partDuration, [sprayEffect2, suit, 0])
    sprayTrack3 = getPartTrack(sprayEffect3, partDelay +
    ...
    toonTrack = getToonTrack(
        ...
        dodgeAnimNames=['sidestep'],
        showMissedExtraTime=0.8)
    return Parallel(suitTrack, toonTrack, sprayTrack1,
...
def doLegalStorm(attack):
    ...
    BattleParticles.setEffectTexture(
        snowEffect, 'legalese-hc', color=effectColor)
    BattleParticles.setEffectTexture(
        snowEffect2, 'legalese-qpq', color=effectColor)
    BattleParticles.setEffectTexture(
        snowEffect3, 'legalese-vd', color=effectColor)
    cloud = globalPropPool.getProp('stormcloud')
    partDelay = {
        'a': 1.2,
        'b': 1.2,
    ...
    toonTrack = getToonTrack(
        attack,
        damageDelay=damageDelay[suitType],
        splicedDamageAnims=damageAnims,
        dodgeDelay=dodgeDelay[suitType],
        dodgeAnimNames=['sidestep'],
        showMissedExtraTime=1.1)
    soundTrack = getSoundTrack('SA_brainstorm.ogg', delay=2.6, node=suit)
    return Parallel(suitTrack, toonTrack, cloudPropTrack, soundTrack)
